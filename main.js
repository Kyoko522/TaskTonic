//local storage start
const itemArray = localStorage.getItem("new-task-input")
  ? JSON.parse(localStorage.getItem("new-task-input"))
  : [];

const workingon = localStorage.getItem("works")
...
function drag() {
  ...
  draggables.forEach((task) => {
    //go through all the items on the screen that can be dragged
    ...
    task.addEventListener("dragend", () => {
      //see when the user stops dragging the task
  ...
  function getDragAfterElement(tasks_todo, y) {
    //get the mouse postion
    const taskElements = [
      ...tasks_todo.querySelectorAll(".task:not(.dragging)"),
    ]; //get all the elements inside of the container and put it inside of a array

    return taskElements.reduce(
      (closest, child) => {
        //call a function on the array that we made and loop through it and which element is right after the y postion
        ...
        if (offset < 0 && offset > closest.offset) {
          //we only care about the offset that below 0 and also which one is the closest to zeor
    ...
    ).element; //set the offset to that all the other elements will be closer to y and is the smallest
  }
}
...
window.addEventListener("load", () => {
  //does all the html and css file stuff firtst then come here
...
//this line grabs the value of your stuipd input field and logs it
  ...
  source.forEach((element) => {
	element.addEventListener("drag", (event) => {
		const inputElement = element.querySelector('.content input');
		const inputValue = inputElement.value;
	  console.log("dragging", inputValue);
	});
  });
  form.addEventListener("submit", (e) => {
    //run this every time the submit button is clicked
    ...
    //this is equal to having draggable = 'true'
    task_el.className = "task";
    task_el.classList.add("task");
